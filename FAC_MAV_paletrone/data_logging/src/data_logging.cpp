...
geometry_msgs::Vector3 opti_position_human;
geometry_msgs::Vector3 opti_position_palletrone;
...
void publisherSet();
void opti_human_Callback(const geometry_msgs::PoseStamped::ConstPtr& msg);
void opti_palletrone_Callback(const geometry_msgs::PoseStamped::ConstPtr& msg);
void reference_yaw_Callback(const std_msgs::Float32& msg);
...
//	ros::Subscriber filtered_bias_gradient_log=nh.subscribe("/filtered_bias_gradient",1,filtered_bias_gradient_callback,ros::TransportHints().tcpNoDelay());
	...
	ros::Subscriber external_force_z_handle_sub=nh.subscribe("/external_force_z_handle",1,handle_callback, ros::TransportHints().tcpNoDelay());
    	ros::Subscriber opti_human_sub = nh.subscribe("/optitrack/Human_head/poseStamped", 1, opti_human_Callback, ros::TransportHints().tcpNoDelay());
    	ros::Subscriber opti_palletrone_sub = nh.subscribe("/optitrack/Palletrone/poseStamped", 1, opti_palletrone_Callback, ros::TransportHints().tcpNoDelay());
    	ros::Subscriber reference_yaw_sub = nh.subscribe("/T_y_r", 1, reference_yaw_Callback, ros::TransportHints().tcpNoDelay());
...
void publisherSet()
{
	...
	data_log.data[86]=reference_yaw;
	...
	data_log.data[103]=opti_position_human.x;	
	data_log.data[104]=opti_position_human.y;	
	data_log.data[105]=opti_position_human.z;	
	data_log.data[106]=opti_position_palletrone.x;	
	data_log.data[107]=opti_position_palletrone.y;	
	data_log.data[108]=opti_position_palletrone.z;	
...
void opti_human_Callback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    opti_position_human.x = msg->pose.position.x;
    opti_position_human.y = msg->pose.position.y;
    opti_position_human.z = msg->pose.position.z;
}

void opti_palletrone_Callback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{

    opti_position_palletrone.x = msg->pose.position.x;
    opti_position_palletrone.y = msg->pose.position.y;
    opti_position_palletrone.z = msg->pose.position.z;
}

void reference_yaw_Callback(const std_msgs::Float32& msg){
	reference_yaw=msg.data;
}
