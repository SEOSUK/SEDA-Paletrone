...
double length_1 = 0.24215;
double length_2 = 0.1759;
double mass_1 = 0.340;
double mass_2 = 0.14;
...
void initParameters()
{
	...
	init_angle = Inverse_Kinematics(init_position);
	init_angle = init_angle + angle_calibration;
    ...
    angle_max << 360 * phi_convert2Radian, 360 * phi_convert2Radian;
    angle_min << -360 * phi_convert2Radian, -360 * phi_convert2Radian;

    angle_max += angle_calibration;
    angle_min += angle_calibration;

	deadzone_max << 0, 0;
	deadzone_min << 0, 0;
	
	EE_vel_limit = 0.1;  // [m/s] end_effector velocity_limit
	angle_vel_limit = 1;  // [rad/s] angle_vel_limit

	phi_ref << 0, 0;


	angle_calibration << -PI/2, 0;
}
...
{
    angle_meas[0] = msg.position.at(1);
    angle_meas[1] = msg.position.at(2);

	angle_meas = angle_meas - angle_calibration;
}
...
//	ros::Subscriber End_Effector_Position_cmd_sub_ = nh.subscribe("/inch/EE_cmd_gui",100,EE_cmd_gui_Callback,ros::TransportHints().tcpNoDelay());
	...
	init_rate.sleep();
	ros::spinOnce();
    first_position = Forward_Kinematics(angle_meas);


	position_reference_vel_limit = first_position;
	position_reference = init_position;
	
	angle_cmd_vel_limit = angle_meas;
	angle_cmd = init_angle;
	// // --초기값 설정용입니다 End
	...
	ros::spinOnce();
	// ROS_INFO("angle_meas \n %lf, %lf",angle_meas[0], angle_meas[1]);
    ...
    End_Effector_Position_meas = Forward_Kinematics(theta_cmd);

	 ROS_INFO("angle_cmd_vel_limit: [%lf][%lf]", angle_cmd_vel_limit[0], angle_cmd_vel_limit[1]);
	 ROS_INFO("angle_meas: [%lf][%lf]", angle_meas[0], angle_meas[1]);
	 ROS_INFO("pos ref: [%lf][%lf]", position_reference_vel_limit[0], position_reference_vel_limit[1]);
	 ROS_INFO("theta_cmd: [%lf][%lf]", theta_cmd[0], theta_cmd[1]);
    End_Effector_Position_meas = Forward_Kinematics(angle_meas);

	 ROS_INFO("pos_FK: [%lf][%lf]", End_Effector_Position_meas[0], End_Effector_Position_meas[1]);

	// ROS_INFO("===================================");

	cmd.position.push_back(0);
	cmd.position.push_back(angle_safe[0] + angle_calibration[0]);
	cmd.position.push_back(angle_safe[1] + angle_calibration[1]);
	...
	test_topic.linear.x = angle_cmd[0];
	test_topic.linear.y = angle_cmd[1];
	test_topic.linear.z = angle_meas[0];
	
	test_topic.angular.x = angle_cmd_vel_limit[0];
	test_topic.angular.y = angle_cmd_vel_limit[1];
	test_topic.angular.z = angle_meas[1];
