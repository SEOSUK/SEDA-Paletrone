...
unsigned long previousMillis = 0; // 이전 시간 저장용 변수
const long interval = 10; // 100Hz 주기 (10ms)

void Read_encoder1() {
    state_rotation1 = digitalRead(A1);
    if (state_rotation1 != state_rotation_past1 && state_rotation1 == 1) {
        counter1 = digitalRead(B1) != state_rotation1 ? counter1 + 1 : counter1 - 1;
    }
...
void Read_encoder2() {
    ...
    if (state_rotation2 != state_rotation_past2 && state_rotation2 == 1) {
        counter2 = digitalRead(B2) != state_rotation2 ? counter2 + 1 : counter2 - 1;
    }
