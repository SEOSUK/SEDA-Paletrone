...
{
    ros::init(argc,argv,"setting_link_2_listener");
    ros::NodeHandle nh;
    ...
    geometry_msgs::Twist Link2_pos;

    double roll, pitch, yaw;

    ros::Publisher Link2_Pub_ = nh.advertise<geometry_msgs::Twist>("/inch/global_link2", 10);
    ...
    while(nh.ok())
    {
        ...
        try
        {
            transformStamped_globalEE = tfBuffer_globalEE.lookupTransform("tf/inch/paletrone", "tf/inch/Link2", ros::Time(0));
        }
        ...
        Link2_pos.linear = transformStamped_globalEE.transform.translation;

        tf::Quaternion quat;
        tf::quaternionMsgToTF(global_frame.pose.orientation, quat);

        tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);
        Link2_pos.angular.x = roll;
        Link2_pos.angular.y = pitch;
        Link2_pos.angular.z = yaw;
        
        Link2_Pub_.publish(Link2_pos);
